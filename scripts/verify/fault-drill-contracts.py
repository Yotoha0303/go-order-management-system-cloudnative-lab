...
def verify_runtime() -> None:
    ...
    require("FAULT_DRILL_OUTPUT_DIR: fault-evidence/worker" in workflow, "Worker evidence target is missing")
    ...
    require("needs.drill.result" in workflow and "DRILL_RESULT" in workflow, "success/failure result is not recorded")
    require("Phase 8.4 runtime fault drills failed" in workflow, "failed main-run evidence is not auditable")
    require("Inspect the uploaded artifact" in workflow, "failure evidence does not direct operators to diagnostics")
...
def verify_contract_workflow() -> None:
    ...
    require("ResolveFaultDrillPath(AnchorsRelativePathToWorkspace|PreservesAbsolutePath)" in workflow, "Worker workspace-path regressions must execute")
...
def verify_drill_implementations() -> None:
    ...
    require("resolveFaultDrillPath" in worker_test, "Worker evidence-path resolver is missing")
    require('os.Getenv("GITHUB_WORKSPACE")' in worker_test, "relative Worker evidence must anchor to the Actions workspace")
    require("filepath.IsAbs" in worker_test and "filepath.Join(workspace, resolved)" in worker_test, "Worker workspace-path resolution is incomplete")
    require("TestResolveFaultDrillPathAnchorsRelativePathToWorkspace" in worker_test, "Worker workspace-path regression test is missing")
    require("os.MkdirAll(filepath.Dir(path)" in worker_test, "Worker JSON evidence parent directory must be created")
