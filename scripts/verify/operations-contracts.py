...
def verify_runtime_workflow() -> None:
    ...
    require("measured_requests\"] <= 15000" in workflow, "runtime total measured-request assertion is missing")
    ...
    require('--measurement-complete-file "${stop_file}"' in workflow, "load driver must publish the exact measured-stage completion marker")
    require('touch "${stop_file}"' not in workflow, "shell-side completion touch must not extend the measured interval")
    ...
    require("report['healthy_measured_errors']" in workflow, "Issue summary must use healthy pre-boundary errors")
    require("Measured errors across all bounded stages" not in workflow, "Issue summary must not present all-stage errors as healthy evidence")
    require("Errors in healthy sustained stages before the first boundary" in workflow, "healthy pre-boundary error evidence claim is missing")
    require("Raw all-stage requests and errors remain in the artifact" in workflow, "raw all-stage diagnostic boundary is missing")
    ...
    require("RabbitMQ session evidence preserves each scraped replica" in workflow, "per-replica RabbitMQ evidence claim is missing")
...
def verify_load_tooling() -> None:
    ...
    require("MAX_TOTAL_MEASURED_REQUESTS = 15000" in driver, "total measured-request cap is missing")
    require("args.max_requests_per_stage * len(levels) > MAX_TOTAL_MEASURED_REQUESTS" in driver, "custom profiles can bypass the total request cap")
    require("maximum total measured requests must not exceed" in driver, "total request-cap rejection is missing")
    ...
    require("write_timestamp_marker" in driver and "measurement_started_at" in driver and "measurement_finished_at" in driver, "measured interval markers are incomplete")
    require("measurement_complete_file" in driver, "driver-side completion marker is missing")
    ...
    require("test_validate_args_enforces_total_request_cap" in tests, "total request-cap test is missing")
    require("test_measurement_start_marker_creates_parent_and_timestamp" in tests, "measurement-start regression test is missing")
    require("write_timestamp_marker(marker)" in tests, "timestamp marker regression uses a stale helper name")
    ...
    require("healthy_error_count" in analyzer, "healthy pre-boundary error calculation is missing")
    ...
    require("test_healthy_error_count_excludes_boundary_and_later_stages" in analyzer_tests, "healthy error-boundary regression test is missing")
    ...
    require("'go_order_rabbitmq_session_up'" in capture, "RabbitMQ session evidence must preserve raw per-replica series")
    require("max by (job,role) (go_order_rabbitmq_session_up)" not in capture, "RabbitMQ session evidence must not collapse replicas")
