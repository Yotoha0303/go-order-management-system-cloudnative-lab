...
- 商品页面覆盖创建、状态筛选、服务端分页、详情、上架和下架
- 库存与流水页面使用商品选择器完成查询和操作，不要求手输商品 ID
- 订单页面仅选择已上架商品，覆盖创建、列表、详情、支付、完成和取消
- 前端对网络重试复用订单幂等 Key，并对取消、完成操作二次确认
- 个人资料与账号安全页面分别对接昵称修改和密码修改
...
- 商品上架后 status = 1
- 商品下架后 status = 2
- 查询商品时默认查询下架商品；支持 status = 1、2、all 和可选分页参数
